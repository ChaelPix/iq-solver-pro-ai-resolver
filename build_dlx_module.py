...
def build_dlx_module():
    """
    Build the Dancing Links (DLX) C module for optimized Algorithm X.
    """
    print("Building DLX module...")
    ...
    c_file = os.path.join(src_dir, 'dlx_solver.c')
    
    # Path for output library
    output_file = os.path.join(src_dir, 'dlx_solver.so')
    ...
    # Use more aggressive optimization flags for better performance
    if system == 'Darwin':  # macOS
        compile_cmd = [compiler, '-shared', '-fPIC', '-O3', '-march=native', '-flto', '-Wall', c_file, '-o', output_file]
    elif system == 'Linux':
        compile_cmd = [compiler, '-shared', '-fPIC', '-O3', '-march=native', '-flto', '-Wall', c_file, '-o', output_file]
    elif system == 'Windows':
        ...
        if not check_compiler_exists(compiler):
            ...
            return False
        output_file = os.path.join(src_dir, 'dlx_solver.dll')
        compile_cmd = [compiler, '-shared', '-O3', '-march=native', '-flto', '-Wall', c_file, '-o', output_file]
    else:
    ...
    print(f"Successfully built DLX module: {output_file}")
    return True

if __name__ == "__main__":
    success = build_dlx_module()
